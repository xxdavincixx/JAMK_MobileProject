...
local function spawnWall(x,y,w,h)                   -- create a wall 
    ...
    local wallCollisionFilter = {categoryBits=1, maskBits=31}                               -- collision filter categoryBits means which number the object ist, maskBits is with which object this object will collide
...
local function spawnPlatform(x,y,w,h)               -- create a platform a player can get through by jumping at x-position (x) and y-position (y) with width (w) and height (h)
    ...
    local platformCollisionFilter = {categoryBits = 4, maskBits = 24}        -- create collision filter, own value = 4 and collides with the sum of values equal to 8 (player_ghost)
...
local function spawnIncreasingObject(x,y)
    math.randomseed(os.time())
    local xRand, yRand = math.random(), math.random()
    local object = display.newRect(x,y,20,20)
    local objectCollisionFilter = {categoryBits = 16, maskBits = 13}
    physics.addBody(object,"static", {bounce = 0.1, filter = objectCollisionFilter})
    object.collType = "increase"
    object:setFillColor(1,0.5,0.2)
    return object
end

local function spawnDecreasingObject(x,y)
    math.randomseed(os.time())
    local xRand, yRand = math.random(), math.random()
    local object = display.newRect(x,y,20,20)
    local objectCollisionFilter = {categoryBits = 16, maskBits = 13}
    physics.addBody(object,"static", {bounce = 0.1, filter = objectCollisionFilter})
    object.collType = "decrease"
...
function scene:create( event )                        
    ...
    floor = spawnPlatform(450,320,900,30)                       -- components
    platform = spawnPlatform(60,200,80,10)                      -- 
    platform1 = spawnPlatform(220,200,80,10)
    increaseObject = spawnIncreasingObject(890,110)
    increaseObject1 = spawnIncreasingObject(969,245)
    increaseObject2 = spawnIncreasingObject(643,157)
    increaseObject3 = spawnIncreasingObject(827,12)
    decreaseObject = spawnIncreasingObject(337,253)
    decreaseObject1 = spawnDecreasingObject(817,112)
    decreaseObject2 = spawnIncreasingObject(86,227)
    decreaseObject3 = spawnDecreasingObject(734,60)
    platform2 = spawnPlatform(460,200,80,10)
    platform3 = spawnPlatform(700,200,80,10)
    platform4 = spawnPlatform(940,200,80,10)
    finishPlatform = spawnPlatform(950,310,100,10)
    finishPlatform.typ = "finish"
    ...
    camera:add(increaseObject,2)
    camera:add(increaseObject1,2)
    camera:add(increaseObject2,2)
    camera:add(increaseObject3,2)
    camera:add(decreaseObject,2)
    camera:add(decreaseObject1,2)
    camera:add(decreaseObject2,2)
    camera:add(decreaseObject3,2)
    camera:add(finishPlatform,2)
...
function scene:show( event )   
    ...
    if event.phase == "did" then
        ...
        function player_ghost:enterFrame()                                      -- each frame
            if(player.didFinish == true)then
                ...
                composer.removeScene("winning")
                composer.gotoScene("winning", { time= 500, effect = "crossFade" })
            end
   ...
   function onPreCollision( self, event )
        ...
        if(collideObject.typ == "finish")then
            player.didFinish = true
        end


        if(collideObject.collType == "increase" and collideObject.alpha == 1) then
            increase_fps()
            collideObject.alpha = 0
            timer.performWithDelay(1, function() physics.removeBody( collideObject ) end )
        elseif(collideObject.collType == "decrease"  and collideObject.alpha == 1) then
            decrease_fps()
            collideObject.alpha = 0
            timer.performWithDelay(1, function() physics.removeBody( collideObject ) end )
        end
